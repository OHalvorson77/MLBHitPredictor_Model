...
data = statcast(start_dt="2025-05-24", end_dt="2025-03-01")
...
def get_stadium_features(venue_name):
    ...
    return {
        ...
        "center": attrs.get("center", 400),
        "left": attrs.get("left", 330),
        "right": attrs.get("right", 330),
    }
...
for game in games:
    ...
    if boxscore['liveData']['boxscore']['teams']['home']['battingOrder'] is None:
        print(game)
        print("Not Ready")
    ...
    for team_type in ['home', 'away']:
        team_info = boxscore['gameData']['teams'][team_type]
        opponent_type = 'away' if team_type == 'home' else 'home'
        opponent_info = boxscore['gameData']['teams'][opponent_type]

        team_id = team_info['id']
        team_name=team_info['name']
            
        if boxscore['liveData']['boxscore']['teams'][opponent_type]['pitchers'] ==[]:
            continue 
        starter_id = boxscore['liveData']['boxscore']['teams'][opponent_type]['pitchers'][0]
        bullpen_ids =  boxscore['liveData']['boxscore']['teams'][opponent_type]['bullpen']

        lineup_player_ids = boxscore['liveData']['boxscore']['teams'][team_type]['battingOrder']
...
for game_id, group in df.groupby('game_id'):
    if len(group) != 2:
        print(f"Skipping game {game_id} due to unexpected team count")
        continue
    ...
    row = {
        'game_id': team1['game_id'],  # Extract just the gamePk
        'home_name': team1['team_name'],
        'team_2': team2['team_name'],
        'team_1_score': team1['predicted_score'],
        'team_2_score': team2['predicted_score'],
        'total_score': team1['predicted_score'] + team2['predicted_score']
    }
...
API_KEY = 'you are not finding out :)'
...
try:
    ...
    for game in events:
        ...
        print(f"Processing event ID: {event_id} {home_name}{away_name}")

        newParams = {
            'apiKey': API_KEY,
            'eventId': event_id,
            'bookmakers': 'Bet365,Betano,Lottoland,BetVictor'
        }
        ...
        if bookmakers:
            for book in bookmakers:
                book_odds = bookmakers['Bet365']

                # bet365_odds is a list — find the totals entry
                totals_entry = next((item for item in book_odds if item.get('name') == 'Totals'), None)
...
merged_df = pd.merge(df, summary_df, on='home_name', how='inner')

merged_df.to_csv("recent_load.csv", index=False)
